...
def setup_3d_axes_safe(ax, center_point=None, range_meters=2.0, azim=45, elev=20):
    ...
    # 스켈레톤이 큐브에 맞게 크기 조정 (범위를 줄여서 스켈레톤을 더 크게 표시)
    half_range = range_meters / 2.0  # 기존 2에서 3.5로 변경하여 스켈레톤을 더 크게
    
    # Center the skeleton properly in all axes, including Z
    ax.set_xlim(x_center - half_range, x_center + half_range)
    ax.set_ylim(y_center - half_range, y_center + half_range)
    ax.set_zlim(max(0, z_center - half_range), z_center + half_range)  # Z축은 0 이상으로 유지
    ...
    # 바닥 표시를 위한 평면 추가
    xx, yy = np.meshgrid(
        np.linspace(x_center - half_range, x_center + half_range, 10),
        np.linspace(y_center - half_range, y_center + half_range, 10)
    )
    zz = np.zeros_like(xx)
...
def _apply_axis_transform(k3d, order='xyz', flip_z=False):
    ...
    # Azure Kinect 좌표계에서 사람이 서있는 방향으로 수정
    # Z축을 위쪽으로, Y축을 앞쪽으로 맞추기
    if order == 'xzy':
        # Y와 Z를 바꾸고 Z를 뒤집어서 머리가 위로 오도록 함
        temp = xyz.copy()
        xyz[:, 0] = temp[:, 0]  # X는 그대로
        xyz[:, 1] = -temp[:, 2]  # Y = -Z (깊이를 Y로)
        xyz[:, 2] = temp[:, 1]   # Z = Y (높이를 Z로)
...
def _reverse_axis_transform(k3d, order='xyz', flip_z=False):
    ...
    # Azure Kinect 좌표계 변환 원복
    if order == 'xzy':
        temp = arr.copy()
        arr[:, 0] = temp[:, 0]   # X는 그대로
        arr[:, 1] = temp[:, 2]   # Y = Z (높이에서 Y로)
        arr[:, 2] = -temp[:, 1]  # Z = -Y (Y에서 깊이로)
    else:
        # 기본 축 순서 원복
        idx_map = {
            'xyz': (0, 1, 2),
