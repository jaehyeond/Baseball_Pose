...
# -*- coding: utf-8 -*-
"""
Two-Camera 3D Triangulation (Multi-person, supports Force-17)
...
Usage (example):
python triangulate_3d.py ^
  --calib-file D:\mmpose\calibration\azure_kinect_calibration.json ^
  --cam1-poses D:\mmpose\poses_2d_cam00 ^
  --cam2-poses D:\mmpose\poses_2d_cam01 ^
  --output-file D:\mmpose\output_3d\azure_kinect_3d_results.json ^
...
# Calibration / I/O
# ----------------------------
def load_calibration(calib_file, extrinsic_format='c2w'):
    """Load camera intrinsics/extrinsics and build projection matrices P=K[R|t].
       extrinsic_format: 'c2w' (camera-to-world in JSON) or 'w2c' (world-to-camera in JSON)
    """
    ...
    for cam_data in calib_data['cameras']:
        name = str(cam_data['name'])  # e.g., '00', '01'
        K = np.array(cam_data['K'], dtype=np.float64)
        dist = np.array(cam_data.get('distCoef', [0, 0, 0, 0, 0]), dtype=np.float64).reshape(-1, 1)
        R = np.array(cam_data['R'], dtype=np.float64)
        t = np.array(cam_data['t'], dtype=np.float64).reshape(3, 1)

        if extrinsic_format == 'c2w':
            # JSON is Camera->World; convert to World->Camera
            R_wc = R.T
            t_wc = -R_wc @ t
            P = K @ np.hstack([R_wc, t_wc])
        else:
            # JSON is already World->Camera
            P = K @ np.hstack([R, t])

        cameras[name] = {
            'K': K, 'distCoef': dist, 'R': R, 't': t, 'P': P,
...
def load_2d_instances(json_file):
    """
    Load all person instances from a MMPose-like pose JSON.
    Expected structure (per frame):
      {
        "instance_info": [
...
def main():
    ...
    parser.add_argument('--calib-file', required=True, help='Calibration JSON with cameras[ {name,K,distCoef,R,t} ]')
    parser.add_argument('--cam1-poses', required=True, help='Folder of results_*.json for camera 00')
    parser.add_argument('--cam2-poses', required=True, help='Folder of results_*.json for camera 01')
    ...
    parser.add_argument('--extrinsic-format', choices=['c2w', 'w2c'], default='c2w',
                        help='Extrinsic convention in calibration JSON')
    ...
    cameras = load_calibration(args.calib_file, extrinsic_format=args.extrinsic_format)
    cam_names = ['00', '01']
